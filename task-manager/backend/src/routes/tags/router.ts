...
const TagRouter = Router()
  .get('', async (_request: Request, response: Response<WithId<Tag>[]>): Promise<any> => {
  ...
  .post(
    ...
    async (request: Request, response: Response): Promise<any> => {
      ...
      console.error(`${getLoggingPrefix()} MongoDB did not acknowledge the tag creation request`);
  ...
  .delete(
    ...
    async (request: Request, response: Response): Promise<any> => {
      ...
      console.error(`${getLoggingPrefix()} MongoDB didn't acknowledge the deletion request for tag with ID ${id}`);
  ...
  .patch(
    ...
    param('id')
      ...
      .isString()
      .withMessage('Tag ID must be a string'),
    body('name').optional().isString().withMessage('Tag name must be a string'),
    body('color')
      .optional()
      .isInt(COLOR_MIN_MAX)
      .withMessage('Color must be a valid 24-bit RGB value'),
    ...
    async (request: Request, response: Response): Promise<any> => {
      ...
      console.error(`${getLoggingPrefix()} MongoDB didn't acknowledge the update request for tag with ID ${id}`);

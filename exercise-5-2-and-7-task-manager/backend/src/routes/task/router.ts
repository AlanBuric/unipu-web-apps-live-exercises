...
const TaskRouter = Router()
  .get(
    '',
    async (_request: Request, response: Response): Promise<any> =>
      Promise.all([
        getDatabase().collection<Task>('tasks').find({userId: response.locals.userId }).toArray(),
        getDatabase().collection<Tag>('tags').find().toArray(),
  ...
  .post(
    ...
    async (request: Request, response: Response): Promise<any> => {
      ...
      task.userId = response.locals.userId;
      task.done = false;
          ...
          .then((docs) => {
            if (docs.length !== tagIds.length) {
              throw new RequestError(
                ...
                'Not all tag IDs exist, or duplicate were provided'
              );
            }
          });
      }

      const result = await getDatabase().collection<Task>('tasks').insertOne(task);

      if (result.acknowledged) {
        return response.status(StatusCodes.CREATED).send(result.insertedId);
      }

      console.error(`${getLoggingPrefix()} MongoDB didn't acknowledge the task saving request for a new task`);
  ...
  .patch(
    ...
    body('done').optional().isBoolean().withMessage('Task done status must be a boolean'),
    body('tagIds')
      .optional()
      .isArray(TAG_IDS_ARRAY_MIN_MAX_OPTIONS)
      .withMessage(`A task may have up to ${TAG_IDS_ARRAY_MIN_MAX_OPTIONS.max} tags`),
    ...
    async (request: Request, response: Response): Promise<any> => {
      ...
      if (task.tagIds) {
        ...
        if (tags.length !== tagIds.length) {
          throw new RequestError(
            StatusCodes.NOT_FOUND,
            'Not all tag IDs exist, or duplicates were provided'
          );
        }
  ...
  .delete(
    ...
    async (request: Request, response: Response): Promise<any> => {
      ...
      const result = await getDatabase()
        .collection<Task>('tasks')
        .deleteOne({ _id: new ObjectId(id), userId: response.locals.userId });

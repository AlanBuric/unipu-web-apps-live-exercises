...
describe("POST /order", () => {
  ...
  ])("should PATCH /pizza/:id successfully", async (testCase) => {
    const response = await testAgent.patch(`/pizza/${testCase.id}`).send(testCase.in);

    expect(response.status).toBe(StatusCodes.OK);
    expect(response.body).toEqual(testCase.out);
  })
});
